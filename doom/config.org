...
>>>>>>> 76db0e1 (emacs :: improved completion binndings)
...
* editor
- evil-mode :: want it everywhere => enable evil mode in minibuffers (consistency!)
#+begin_src emacs-lisp
...
- instant trigger :: make completion consistent with vim, when activated the completion is triggered and needs no confirmation (~company-tng-mode~).
