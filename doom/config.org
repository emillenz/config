...
- simplified, fast window & buffer navigation (vim makes it overly complex: window-left/right/up/down, kill-buffer/close-window -> next/prev, quit)
  - c-w is all you need, since if you have more than 2 splits open at a time you are doing something wrong anyways
...
- buffers ::
  - use last-buffer [[kbd:C-^][C-^]]  to alternate/go-back to last and find-buffer.  [[kbd:C-g][C-g]] for pulling up arbitrary buffers
...
- terminal :: we never use an integrated terminal (introduces additional complex layers, slowness, bad keybindings, etc...).  instead all we do is use [[kbd:M-!, M-&, M-|]] if more is needed than that we should switch over to the proper shell anyway.
