...
- buffers ::
  - use last-buffer [[kbd:C-^][C-^]]  to alternate/go-back to last and find-buffer [[kbd:M-g][M-g
]] for pulling up arbitrary buffers
...
- [[kbd:m][m]] :: save point to jumplist (save pos -> do edits elsewhere -> jump back to saved pos: ~c-o~)
...
- [[kbd:c-l][c-l]] does the same as [[kbd:c-l][c-l]] in the shell.  is quicker & more mnemonic than [[kbd:zz][zz]]
- learn them all (also the one's in insert mode) for seizing vim's full power.
- managing buffers ::
- ~save-buffer~ & ~kill-buffer~ are statistically in the same usage-frequency-class as editing commands, and exist fundamentally in all programs in some disguise. => mapped to consisten [[kbd:C-s][C-s]]
- completion :: select & activate: [[kbd:C-j/k][C-j/k]], superior to [[kbd:TAB][TAB]] since it is faster & to navigate between entries.  confirm/exit: [[kbd:RET][RET]]
