...
- Redefine ~evil-quit~ to close the tab before closing the frame.
#+begin_src emacs-lisp
...
- Useless default mappings :: remap underused/useless keys to statistically frequently used commands
  ...
  - ~M~: save point to jumplist (save pos -> do edits elsewhere -> jump back to saved pos: ~C-p~)
  - ~&~: Allows for same usage as normal: ~&~, but more powerful (repeat any last ~ex~ command)
  - ~TAB~: folding (inspired by org-modes folding (smart/simple one-key folding))
...
- Screen movements ::
  ...
  - Navigate by sections :: Paragraph nav: ~C-j/k~
  - Map it to the os-consistent key
- ~C-l~ does the same as ~C-l~ in the shell and is quicker & more mnemonic than ~zz~.
- Marks :: set a mark for the jumplist with ~c-m~ at current position
  - This has basically replaced my need for vim's marks as i mostly alternate between two positions.
#+begin_src emacs-lisp
