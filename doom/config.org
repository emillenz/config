...
#+macro: src-len (eval (save-excursion (find-file (file-name-concat doom-user-dir "config.el")) (count-lines (point-min) (point-max))))
...
** completion
- completion saves time, hence we want it fast, and enabled everywhere except for prose.
#+begin_src emacs-lisp
...
- daily journal ::
  ...
  - use for random thoughts and ideas during the day that you want to capture and preserve (be able to look back on)
