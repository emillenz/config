...
** tabs
- assign buffers to tabs in order to hotswitch between them (say you are in a project and need to hop between multiple files alot).
- tabs stay alive, only the buffer's in them change
- if working on miscelaneous things just use buffer searching (faster, less time spent on managing the tabs)
- we don't visual indication for tabs.  instead the switching should be muscle memory => truly be fast and have no mental overhead when context switching.
- we initilize 4 tabs for assignments.  this is the optimum size for most projects.
