...
- Capture templates are used to collect & capture notes, events, tasks and your daily reflection journal structured, tagged, sorted into a specific files.
...
- For each project there is a separate folder (relative to org-dir) with files:
  - tasks :: all tasks (todos, completed etc) and events (physical appointments)
