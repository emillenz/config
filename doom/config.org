...
- Useless default mappings :: remap underused/useless keys to statistically frequently used commands
  ...
  - ~M~: scratch mark: ~m~ (save pos: ~mm~ -> do edits elsewhere -> jump back to scratch: ~M~)
...
- Escape :: Remap ~esc~ to ~left-alt~ systemwide for ergonomic access. (don't map to ~capslock~, since ~caps~ should be mapped to: ~backspace~. Which is even more frequently used and even less ergonomic than ~esc~)
- Whichkey :: the concept of whichkey is in my opinion causing bad habits and redundant
  - Since all functions often called (ie. bound to keys) should be engrained in muscle memory and not tediously looked up in this menu.
  - If you don't know what you are looking for, then use ~M-x~ and fuzzy search for what you think you want to do and it will show you the binding for next time. This is more efficient.
  - Additionally we remove annoying unwanted popup's (distractionless, focused workflow).
...
- ~C-l~ does the same as ~C-l~ in the shell and is quicker & more mnemonic than ~zz~.
#+begin_src emacs-lisp
