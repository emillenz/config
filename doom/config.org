...
* UI
** Theme: ok-solar (dark)
- Oksolar :: Slightly improved contrast between fg/bg vs normal Solarized theme (using modern OKLab colorspaces).
...
** Modus theme
 - Note :: make ~tab-bar~ consistent with completion interface.
#+begin_src emacs-lisp
(setq
...
- Moving by Sections :: ~[]~, implemented in all relevant modes (org, magit..)
...
- Navigate by sections :: ~C-j/k~ (implemented sensibly for other tools)
  - Map it to the os-consistent key
- ~C-l~ does the same as ~C-l~ in the shell and is quicker & more mnemonic than ~zz~.
- Marks :: set a mark for the jumplist with ~M~ at current position
...
- Jumplist :: add more movement commands
#+begin_src emacs-lisp
