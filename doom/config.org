...
** [&] Theme: ok-solar (dark)
:LOG:
- state:     -> "[&]" [2024-01-27 Sat 21:08] \\
  - Under review, as currently modus themes seem superior to me in terms of functionality and design process.
  - Made by my favorite emacs wizard: Protesilaos Stavrou
:END:
...
** Modus theme
 - Make ~tab-bar~ consistent with completion interface.
 - Cursor :: Different cursor color dependent on mode you are in.
 - Emphasis :: we don't want static elements such as the ~tab-bar~ to have much empasis and take away from the focus of the document (thus black on grey, emphasizing current tab with bg).
#+begin_src emacs-lisp
