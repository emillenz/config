...
(defadvice! prompt-for-buffer (&rest _)
  "Prompt for buffer/dir to open after splitting/newtab."
...
- ~()~ are near useless and ~{}~ is too dependent on how the author formatted his code (always use relative jumps and ~C-u/d~ for bigger vertical movement)
  + remap to quick/smart jump forward/backward/out/in delimiters and expressions.
...
- Ctrl-bindings are for: Screen movements
  ...
  + (~z~ is exclusively for folding) => ~zz~ -> ~C-z~ (mnemonic: center)
#+begin_src emacs-lisp
(after! evil
  (map!
...
- These ligatures are bad for code editability
#+begin_src emacs-lisp
