...
* UI
** Theme: ok-solar (dark)
- Oksolar :: Slightly improved contrast between fg/bg (using OKLab).
- Note :: change cursor etc colors based on the theme dynamically and don't hard-code them in (extensible).
...
- <esc> :: Remap <ESC> to <left-alt> systemwide instead.
- whichkey :: the concept of whichkey is in my opinion causing bad habits. Since all functions often called (ie. bound to keys) should be remembered in muscle memory and not tediously looked up in this menu. If you don't know what you are looking for, then use ~M-x~ and it will show you the binding for next time. Additionally we don't want annoying popup's all the time (distractionless workflow). This is for the same reason as you should use a keyboard with blank keycaps => will force you to learn things by muscle memory, (touch type even special characters) to improve the speed exponentially in the long run.
