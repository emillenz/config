...
- i encourage you to master the tool that you are using.  this means reading the documentation, keybinding cheatsheets for the tools you are using (ex: vertico).  additionally when something is repetetive / unefficcient, look up if there exists a command to do what you need to get done.
  + try to first fundamentally understand & master the functionality your tool offers.  then you can assess and see if a extension really provides value to you, or wether that functionality is already included.
    + eg: all the plugins like vim easymotion, avy-goto-char etc. are bloat and not needed, since a faster, more powerful, inter-line function already exitsts: incremental-search: [[kbd:/, ?][/, ?]]
...
- buffers ::
  - use [[kbd:m-b][m-b]] to switch [B]ack and forth between last [b]uffer and current [b]uffer.  for all other buffer navigation needs use: [[kbd:m-g][m-g]] for pulling up an arbitrary buffer.
- tabs ::
  - misc workspace: [1] :: editing various different files, switching between them with [[kbd:m-g][m-g]]
  - project files [2..4] ::
    - open buffers in specific tabs (one file per tab), in order to hotswitch between them.  (ex: working on 3 project files) => one maximized buffer open per tab
...
- efficiency :: this minimalistic but powerful navigation workflow is the most efficient & overheadfree way of navigating between multiple different files/buffer at the same time (low overhead when context-switching and remaining distractionfree, works from anywhere.  improves typing speed)
- splits :: we never manually create split-windows for buffers, prefer the full screen, full focus, no distractions - workflow.  (no multitasking, no clutter)
  - when emacs creates splits we must handle them => minimal bindings for switching and closing splits: [[kbd:m-q, m-w][m-q, m-w]].
  - [[kbd:m-w][m-w]] and [[kbd:m-q][m-q]] is minimalistic, but all you will ever need to manage buffers and windows, since if you have more than 2 windows side by side, you are screwed anyway and need to clean up.  hence it replaces the gazillion of vim's window shortcuts with a faster, simpler alternative.
- terminal :: we never use an integrated terminal (introduces additional complex layers, slowness, bad keybindings, etc...).  instead all we do is use [[kbd:M-!/&|][M-!/&|]] if more is needed than that we should switch over to the proper shell anyway.
...
see [[file:~/.config/xremap/config.yml][global keyboard remaps]] for additional details.
...
- completion :: select & activate: [[kbd:C-n/p][C-n/p]], superior to [[kbd:TAB][TAB]] since it is faster & more ergonomic to navigate between entries.  confirm/exit: [[kbd:RET][RET]]
-
...
- don't add functions to jumplist that are executed repeatedly
#+begin_src emacs-lisp
...
- make the completion-menu less distracting and more focused we use ~vertico-flat-mode~ (it promotes finding items not by navigating via scrolling through candidates, but instead by searching). we don't need a fancy popup everytime we want to switch to a candidate, we only want to know when the completion matches, since we already know beforehand what we are looking for.
#+begin_src emacs-lisp
(after! company
