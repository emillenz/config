...
- buffers ::
  - filemarks :: hotswitch between files in a project you are working on (ex: mark ~Main.java~ with [[kbd:M][M]] and then always jump back to it using ~'m~ )
  - else :: use [[kbd:C-g][c-g]] and partial completion for all other buffer switching (never use next/pre buffer (inefficient))
- efficiency :: this minimalistic but powerful navigation workflow is the most efficient & overheadfree way of navigating simultaneously between multiple different files/buffer when working within a project (low overhead when context-switching and remaining distractionfree, works from anywhere.  improves typing speed)
#+begin_src emacs-lisp
