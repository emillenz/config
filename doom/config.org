...
- tabs ::
  ...
  - 1 ::
    - scratch workspace, where you'll be working if not inside a project.  does not need to be associated with a specific buffer.
  - 2-4 ::
...
#+end_src
 ...
 - tabs should never be created manually (already available, like workspaces, to assign things to)
#+begin_src emacs-lisp
...
- managing buffers ::
  - ~save-buffer~ & ~kill-buffer~ are statistically in the same usage-frequency-class as editing commands, and exist fundamentally in all programs in some disguise.
    + are mapped top-level
...
- override the otherwise useless ~s~ key with a precise char jump.  no more relative line jumping and then ~f~ or ~t~ jumping inline, or spamming ~hjkl~.  instead just jump to the exact position you need to get to immediately without the mental overhead (of doing both steps).
#+begin_src emacs-lisp
...
** completion
- system :: use ~C-j, C-k~ for completion (more ergonomic & faster than tab)
  - activate completion: ~C-j~
  - confirm: ~Return~
- i don't want company to show up and distract me when i am typing.
  + only popup when i specifically request it (with ~tab~)
  ...
  + for repetetive coding it of course may make sense to disable it.
#+begin_src emacs-lisp
(setq
