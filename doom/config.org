...
- workflow :: this minimalistic but powerful navigation workflow (fuzzy-find, find-file, harpoon, and find-buffer) is the most efficient & overheadfree way of navigating simultaneously between multiple different files/buffer when working within a project (low overhead when context-switching and remaining distractionfree, works from anywhere.  improves typing speed)
  - harpoon :: essentially persistent vim global marks on a per project basis (more powerful, faster and more generalized).  hotswitch between files in a project you are working on simultaneoutsly.  we intentionally use [[kbd:c-1-4][c-1-4]], since using numbers causes less mental overhead and makes no presumptions about the files assigned.  additionally it is consistent with tmux and the webbrowser.  you are restrained to 4 files since that is all you'll need and if you have more than that, you must reprioritize your marks.
