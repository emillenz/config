...
- workflow :: this minimalistic but powerful navigation workflow is the most efficient & overheadfree way of navigating simultaneously between multiple different files/buffer when working within a project (low overhead when context-switching and remaining distractionfree, works from anywhere.  improves typing speed)
  - buffer-marks :: hotswitch between buffers in a project you are working on simultaneoutsly (ex: within a project: mark ~Main.java~ -> ~1~, ~buildscript.sh~ -> ~2~, ~compilation~ -> ~3~).  we intentionally use [[kbd:c-1-4][c-1-4]], since using numbers causes less mental overhead and makes no presumptions about the buffers assigned.  additionally it is consistent with tmux and the webbrowser.
