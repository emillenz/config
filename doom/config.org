...
- For maximized-tabs-workflow, open files in new tab to hotswitch immedidately to.
  + Keep the workflow DRY and automate what you repeatingly do.
- This works for forall file opening functions (fzf finding, dired..)
#+begin_src emacs-lisp
...
- Emacs alredy by default implements it that out of screen jumps center the screen which is sensible
...
- -~HL~ are useless (learn relative linejumps you ape!)
  + but must maintain consistency with all other programs running vim bindings
- Shorcut to ex buffer search & replace with ~\|~
- ~()~ are useless
  + Remap to parens jumping
- ~&~: Allows for samd usage as normal ~&~, but allows repeating any last command.
...
** Ligatures
- I use concise, functional notation alot -> make it look coherent.
#+begin_src emacs-lisp
