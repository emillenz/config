...
- buffers ::
  - global-marks ::
    - we use vim's global marks for efficent and fast buffer navigation.
    - hotswitch between files (buffers) in a project you are working on (ex: within a project: mark ~Main.java~ -> ~m~, ~buildscript.sh~ -> ~b~, ~compilation~ -> ~c~; then hotswitch between these buffers, to whichever one you need.)
