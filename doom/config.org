...
- buffers ::
  - use last-buffer (C-^) to alternate/go-back to last and find-buffer (M-g) for pulling up arbitrary buffers
...
- ~][~ moves by syntactical sections implemented in all relevant modes (org, magit..)
...
- managing buffers ::
- ~save-buffer~ & ~kill-buffer~ are statistically in the same usage-frequency-class as editing commands, and exist fundamentally in all programs in some disguise. => mapped to consisten ~C-s~
...
- use ~f/t~ inline and ~s/S~ when jumping inter-line
#+begin_src emacs-lisp
...
- Completion ::
- use ~C-j, C-k~
- confirm the autocompletion with ~<RET>~.  (consistent with all other programs on the os (neovim, nushell, browser...))
