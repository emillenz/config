...
** Ligatures
- For org-mode, using ~ligatures-extra~
- i use alot of this notation for conciseness -> make it look nicer.
#+begin_src emacs-lisp
...
#+RESULTS:
| t  | task |       |                                    |                 |
| tc | cs   | entry | (file+headline cs/tasks.org inbox) | * [ ] %^{title} %^g |
...
*** [?] Override language indentation (not consistent -> disabled)
#+begin_src emacs-lisp :tangle no
