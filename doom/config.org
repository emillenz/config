...
- buffers ::
  - use [[kbd:C-b][C-b]] to switch [B]ack and forth between last [b]uffer and current [b]uffer.  for all other buffer navigation needs use: [[kbd:C-g][C-g]] for pulling up an arbitrary buffer.
...
- splits :: we never manually create split-windows for buffers, prefer the full screen, full focus, no distractions - workflow.  (no multitasking, no clutter)
  - when emacs creates splits we must handle them => minimal bindings for switching and closing splits: [[kbd:c-q, c-w][c-q, c-w]].
  - [[kbd:c-w][c-w]] and [[kbd:c-q][c-q]] is minimalistic, but all you will ever need to manage buffers and windows, since if you have more than 2 windows side by side, you are screwed anyway and need to clean up.  hence it replaces the gazillion of vim's window shortcuts with a faster, simpler alternative.
...
- limit the showing of number of completion candidates (can't view more than ~8 at a glance anyways) in order to make the completion-menu less distracting and more focused.
#+begin_src emacs-lisp
