...
[[file:~/Documents/compass/software/coding_principles.org][Coding Principles]]
...
- ~C-j~ & ~C-k~ mapped sensible and consistent with other modes (~info~, ~magit~..
...
- Note :: pay attention as to which expressions are (and should) evaluated on config parsing vs the ~:keywords~, which are evaluated upon calling the capture template.
- Note :: this is a bit of an unorthodox solution for org capture templates, but in order to be consistent the tree structure is hierarchical by project and then follow the generic templates.
#+begin_src emacs-lisp
