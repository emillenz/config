...
* Abbreviations of long commands
...
$map XF86MonBrightnessUp   exec sudo xbacklight +5 && notify-send "󰃟 Light" "$(light)%"
$map XF86MonBrightnessDown exec sudo xbacklight -5 && notify-send "󰃟 Light" "$(light)%"
#+end_src
...
mode "$keyboard" {
  $map $mod+d exec setxkbmap us dvorak; $esc
  $map $mod+c exec setxkbmap ch;        $esc
  $map $mod+u exec setxkbmap us;        $esc
  $map Escape $esc
}
#+end_src
