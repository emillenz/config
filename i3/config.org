...
- Lower frequency -> less visually distractiong as not ticking down in statusbar
#+begin_src conf
set $timer_cmd ~/.config/bin/timer --frequency=1m
...
mode "$timer" {
  bindsym $mod+t       exec $timer_cmd toggle;       $esc
  bindsym $mod+r       exec $timer_cmd reset;        $esc
  bindsym $mod+s       exec $timer_cmd start;        $esc
  bindsym $mod+i       exec $timer_cmd start 60m 0m; $esc
  bindsym $mod+shift+i exec $timer_cmd start 60m 5m; $esc
  bindsym $mod+1       exec $timer_cmd start 10m;    $esc
  bindsym $mod+2       exec $timer_cmd start 20m;    $esc
  bindsym $mod+3       exec $timer_cmd start 30m;    $esc
  bindsym $mod+4       exec $timer_cmd start 40m;    $esc
  bindsym $mod+5       exec $timer_cmd start 50m;    $esc
  bindsym $mod+6       exec $timer_cmd start 60m;    $esc
  bindsym $mod+7       exec $timer_cmd start 70m;    $esc
  bindsym $mod+8       exec $timer_cmd start 80m;    $esc
  bindsym $mod+9       exec $timer_cmd start 90m;    $esc
  bindsym Escape $esc
}
#+end_src
