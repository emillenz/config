...
- Focus app if already exists on different workspace -> attach to server -> start server -> lanuch app
- for apps that usually have multiple instances, don't focus existing one, insted launch new (zathura).
#+begin_src  conf
set $open <open> \
│ editor \
│ ^editor-everywhere \
