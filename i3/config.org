...
mode "media" {
        $map $mod+p $cmd playerctl play-pause $esc
...
client.unfocused        $modeline $modeline $fg   $modeline  $bg
client.urgent           $active   $active   $fg   $bg        $active
client.placeholder      $modeline $modeline $fg   $bg        $modeline
client.background       $bg
#+end_src

* Statusbar
- hide it, and show it only to indicate mode or if needed sys-info specifically.
#+begin_src conf
bar {
        mode hide
        hidden_state hide
