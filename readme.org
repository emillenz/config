...
** Colorscheme: Solarized (light variant)
#+begin_quote Solarized reduces brightness contrast but, unlike many low contrast colorschemes, retains contrasting hues (based on colorwheel relations) for syntax highlighting readability.
- Ethan Schoonover
#+end_quote
...
- Light variant :: Looks like as if someone left a sheet of paper out in the sun for far too long and now it is all yellowed. It reminds me of an old worn and torn document, that you can spend hours on end looking at on without getting tired.
...
- Prefer to use light-variant per default, since this is mostly consistent with other applications (+websites) that are not themed and puts less strain on the eyes when reading in ambient/daylight conditions.
  - You should always have ambient light on when coding at night, in order to protect the eyes. Thus using a dark theme makes little sense again.
