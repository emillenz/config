...
most of today's themes are an angry fruit-salad: using as many hues as possible without considering if they fit in context and are even needed.

we use the light theme, if at night, ensure you have enough ambient light for usability ergonomics.  studies & research have found the following advantages of using a light-theme over a dark-theme ::
- significantly better proofreading (catch more errors)
- high legibility at low brightness / in direct sunlight => less eyestrain, more efficient battery usage
- no astigmatism (bloom of the letter's cause them to blur)
...
|---------+-----------------------------+---------------------|
| k/j     | prev/next window (!not tab) | vim, ergonomics     |
| 1-4     | switch to tab by index      | logic / stardard    |
| g       | goto tab/buffer  (find)     | vim: goto           |
| q       | quit (buffer/program)       | mnemonic / standard |
| e       | edit file / editor          | mnemonic, vim ~:e~    |
| f       | find (program/file)         | mnemonic / standard |
| x       | run-command (program)       | emacs: m-x          |
| c       | command-line (shell)        | standard            |
| w       | web (browser)               | mnemonic, www       |
| o       | open (file/program)         | mnemonic / standard |
...
- the wm is directly coupled to the system, hence it makes sense to run shell-commands and scripts instead of something such as ~stumpwm~.
