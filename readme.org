...
- This system is 100% centered at keyboard usage and aims to unify all navigational commands across the terminal, editor, browser, window manager and other possible applications using Vim-bindings in combination with a custom mapping scheme which is used for navigation in the applications.
...
- Consistency: This config aims to be globally consistent in its design, behaviour and control.
  + Which means things like [colorscheme, font, navigation, keybindinsgs] were once carefully analyzed, choosen and designed and then implemented across the tools (terminal, editor, browser, wm, docreader...)
  + Avoid using splits in your programs and prefer the full tab/buffer only workflow. (exception: need to actively monitor two different positions)
    ...
    + Get rid of the large set of now redundant keybindings for manual split management.
...
- Available in (almost) every program, leadermode is not.
...
- All coding, writing, fileopening, viewing, navigating is done inside emacs (do not leave it), because this makes for a consistent and coherent workflow.
  + Never open a terminal-emulator inside emacs (no layering!), if you need, use M-c for single shellcommands (make executeable, etc...). (Things like: compliation...  etc should be done using hotkeys and displaying the output in a buffer)
