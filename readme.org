...
* keybindings
** general notes
- [[kbd:ctrl][ctrl]] :: mapped to [[kbd:caps][caps]] this greatly increases ergonomics and speed for all ctrl-bindings and goes hand in hand with [[kbd:c-w/h][c-w/h]] for deleting words and characters in the shell.
  - esc :: never use the actual [[kbd:esc][esc]] key, instead habit of exiting insert mode using ~c-[~ which is reacheable fast & overheadfree from the homerow.
  - this was the original position of when emacs and vi were developed, when developer's used [[kbd:ctrl][ctrl]] for everything (rub out, navigation, etc..) on the old terminals.
- deleting characters :: make using [[kbd:c-w][c-w]], [[kbd:c-h][c-h]] instead of [[kbd:backspace][backspace]] a habit.  it is more ergonomic and faster than using the incredibly unergonomic and hard to reach [[kbd:backspace][backspace]] (hence we implement these bindings in all apps) (this is inspired from terminals, since there [[kbd:c-<backspace>][c-<backspace>]] doesn't even work to begin with.)
...
- key-repetition :: disable it.  it empowers bad habits of you holding down a key instead of moving intelligently and fast using the appropriate motion (not holding down [[kbd:jk][jk]], instead using [[kbd:c-u/d][c-u/d]].  not spamming delete, instead using [[kbd:d-<motion>][d-<motion>]]).
- keychords
