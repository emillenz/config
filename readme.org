...
- It's functionality and colorchoices it brings is unmatched by any other colorscheme. (Protesilaos Stavrou's: modus-themes are a close runner up.)
- Dark variant :: The background color of the dark variant is what i imagine one would see if they were drowning in the middle of the ocean. To me it represents the color you see when dying. Both light/dark are good for long time terminal and coding sessions.
- Prefer to use dark-variant per default, since this is mostly consistent with other applications (+websites) that are not themed and reduces strain when focusing.
  - You should always have ambient light on when coding at night, in order to protect the eyes.
...
Do we get something usable? Or do we have a gestalt form that constantly reminds the user “hey, look how pretty I am!”? We want the design to help the user. Nothing more, nothing less.
