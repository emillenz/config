...
** shell: fish / ruby
a shell only exists for simple system management (launching processes, etc) and interactive use (filtering, text, redirections, etc, not for heavy data processing).  for any scripting beyond that you should always opt towards a proper scripting language.  i highly believe that any scripting away from interactive use should be done in a proper language.
...
fish has consistent syntax, inbuilt autocompletion and is configured with keybindings to make navigation efficient & overheadfree.
...
when writing scripts one should always use the long options of flags in order to increase code readability and maintaineability (not needing to keep this gazillion of abbreviations in your memory for each individual program).  this makes understanding the script for people who may not be familiar with the programs easier to understand and modify (reduce elitism in free software).  (this does not slow you down at all, this is what an lsp and codecompletion is for).

choosen over python since it is well designed, has a very minimal, clean, consistent syntax.  close interconnectedness with ~sh~ by running ~sh~-commands with ~``~.
i love the fact that everything is a expression (has a value). This means, it's a extreme joy for functional programers.

Python is pretty badly designed, lots inconsistencies. Its syntax is a syntax soup, and its semantics a semantics soup.
...
*** workflow with emacs
- the terminal is used to run their own terminal-based programs and system related commands (mainly tuis and long async-processes) and commands: htop, nmtui, git, metasploit, transmission-cli, ssh.  (and ofc as the system commandline).
  - all other commands, especially one's coupled with programming and file/data manipulation should be run from emacs.
...
* keybindings
implemented using ~xremap~
** general notes
- esc :: remapped ergonomically to [[kbd:capslock][capslock]]
- [[kbd:super][super]] :: mapped to otherwise useless [[kbd:caps]].  this is an essential remap.  it is and ergonomic remap.
- [[kbd:ctrl][ctrl]] :: swapped with [[kbd:alt][alt]], since we use [[kbd:ctrl][ctrl]] much more often as a modifier than alt, and this now allows ergonomic pressing of the ctrl keys (using the opposite hand, the same way you do for shift).
- insert-mode :: make using [[kbd:c-u][c-u]], [[kbd:c-w][c-w]], [[kbd:c-h][c-h]] a habit and ditch the terribly placed [[kbd:backspace][backspace]].  it is more ergonomic and faster than using the incredibly unergonomic and hard to reach [[kbd:backspace][backspace]] (hence we implement these bindings in all apps)
...
- chords :: for repeating/high-usage commands: switch buffer, save, close buffer, etc..  thus we need to be able to hold down the key to repeat the action multiple times.  and with holding down it is quicker to access.
- sequence :: for more infrequent, single call functionality (ex: compile / )
...
- it condenses down a minimal set of universal, efficient navigational commands.
...
- windowmanager :: the set of program-names during runtime is known and consistent (unlike the set of buffers in applications, where tab-names are either unnamed, arbitrary or change during runtime), we use specific-chars for switching to the tabs (similar as vim's per file marks).
- tabs ::
  - use them as hotkey workspace inside an app (ex: emacs / tmux)
  - avoid manual tab creation if possible => auto-create a tab if it doesn't exist upon focusing it
...
- [[kbd:ctrl][ctrl]] :: inside applications since it is the defacto standard & aligns with vim's navigational-ctrl-layer-keybind (and is now ergonomically bound).
- implementation :: emacs, tmux, fish, firefox,... all implement this mapping scheme.

| mod+key | action                       | consistency         |
|---------+------------------------------+---------------------|
| [[kbd:g][g]]       | [g]oto buffer  (find)        | vim: goto           |
| [[kbd:t][t]]       | new-[t]ab                    | standard            |
| [[kbd:s][s]]       | [s]ave file                  |                     |
| [[kbd:q][q]]       | [q]uit (buffer)              | mnemonic / standard |
| [[kbd:e][e]]       | [e]dit (file/url/...)        | standard            |
| [[kbd:f][f]]       | [f]ind (program/file, fuzzy) | mnemonic / standard |
| [[kbd:r][r]]       | [r]ecent (file, fuzzy)       | mnemonic / standard |
| [[kbd:x][x]]       | e[x]ecute-command (internal) | emacs               |
| [[kbd:l][l]]       | [l]ast buffer (a[l]ternate)  | mnemonic, ergo      |
| [[kbd:n][n]]       | [n]ext completion            | vim                 |
| [[kbd:p][p]]       | [p]revious completion        | vim                 |
| [[kbd:tab][tab]]     | complete                     | emacs               |
| [[kbd:spc][spc]]     | leader-key                   | vim / ergonomic     |
