...
- windowmanager :: the set of program-names during runtime is known and consistent (unlike the set of buffers in applications, where tab-names are either unnamed, arbitrary or change during runtime), we use specific-chars for switching to the tabs (same as emacs's per file marks).
- tabs ::
  - use them as hotkey workspace inside an app (ex: tmux, browser) // this is the more simple alternative to marks.
...
| [[kbd:1-4][1-4]]   | focus tab 1-3                | standard            |
