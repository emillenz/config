...
- you don't need visual indication of how to navigate to tabs (tmux/emacs), each specific tab is always on the same keybind, and if not it's faster to search for it than visually navigate/select some list / menu
- the aestetic and efficiency of a decluttered and maximized screen is unmatched.
- the aim is to make switching so ergonomic and instantaneous, engrained into muscle memory, that you are faster that people using, say 2 monitors.
