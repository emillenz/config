...
** general notes
- super / alt :: remapped [[kbd:super][super]] to [[kbd:alt_l][alt_l]] opposite to [[kbd:alt_r][alt_r]] key for ergonomics.
- esc :: remapped ergonomically to [[kbd:capslock][capslock]]
- ctrl :: mapped to otherwise useless [[kbd:caps]].  this is an essential remap.  it is ergonomic and allows fast control key chording.
...
- [[kbd:alt][alt]] :: inside applications since it is available everywhere and (almost) never overrides other bindings, aligns with vim's navigational-ctrl-layer-keybind (and is now ergonomically bound).
...
| [[kbd:n][n]]       | completion-menu [n]ext       | vim                 |
| [[kbd:p][p]]       | completion-menu [p]revious   | vim                 |
